...
def main(argv):
    ...
    trigger_time_test = 100  # Time when the trigger occurs (to align errors)
    ...
    for k in range(n_samples_test):
        inputs[k,trigger_time_test,1] = 1
        # Outputs
    desired_outputs = np.zeros((n_samples_test, sample_size_test, n_output))       
    for k in range(n_samples_test):
        desired_outputs[k,trigger_time_test:] = inputs[k,trigger_time_test,0]
