...
def main(argv):
    ...
    trigger_time_train = 50  # Time when the trigger occurs (to align errors)
    trigger_period_train = 200   # Period of the triggers
        # Test parameters
        # ----------------------------------------------------------------------
    n_samples_test = 100     # Number of samples
    sample_size_test = 10000   # Sample temporal size
    trigger_time_test = 50  # Time when the trigger occurs (to align errors)
    trigger_period_test = 200  # Period of the triggers
    ...
    for k in range(n_samples_train):
        inputs[k,np.arange(trigger_time_train, sample_size_train, trigger_period_train),1] = 1
        # Desired outputs
    desired_outputs = np.zeros( (n_samples_train, sample_size_train, n_output) )       
    for k in range(n_samples_train):
        for t in range(trigger_time_train, sample_size_train, trigger_period_train):
            desired_outputs[k,t:t+trigger_period_train,0] = inputs[k,t,0]
    ...
    for k in range(n_samples_test):
        inputs[k,np.arange(trigger_time_test, sample_size_test, trigger_period_test),1] = 1
        # Desired outputs
    desired_outputs = np.zeros( (n_samples_test, sample_size_test, n_output) )       
    for k in range(n_samples_test):
        for t in range(trigger_time_test, sample_size_test, trigger_period_test):
            desired_outputs[k,t:t+trigger_period_test,0] = inputs[k,t,0]
