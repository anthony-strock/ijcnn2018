...
def main(argv):
    ...
    parser.add_argument("-r", nargs = "?", type = str, help = "path to the file where the real output will be saved")
    ...
    for k in range(n_samples_train):
        for l in range(n_trigger_train):
            desired_outputs[k,trigger_times[k,l]:,0] = inputs[k,trigger_times[k,l],0]
            desired_outputs[k,trigger_times[k,l]:,1] = inputs[k,trigger_times[k,l],0]*inputs[k,trigger_times[k,l]:,0]
    ...
    inputs = np.empty( (n_samples_test, sample_size_test, n_input) )
            # Data to store
    random_inputs = rstate.uniform(low = -1., high = 1., size = (n_samples_test, sample_size_test+M-1))
    for i in range(n_samples_test):
        inputs[i,:,0] = signal.convolve(random_inputs[i], window, mode = "valid")
            # Triggers
    inputs[:,:,1] = 0
    trigger_times = np.empty((n_samples_test, n_trigger_test), dtype = np.int64)
    for k in range(n_samples_test):
        trigger_times[k] = np.sort(rstate.permutation(np.arange(trigger_earliest_test, trigger_latest_test))[:n_trigger_test])
        inputs[k,trigger_times[k],1] = 1
        # Desired outputs
