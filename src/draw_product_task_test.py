...
def main(argv):
    ...
    parser.add_argument("inputs", nargs = 1, type = str, help = "path to the file where the input is saved for the once condition")
    parser.add_argument("outputs", nargs = 1, type = str, help = "path to the file where the output is saved for the periodic condition")
    parser.add_argument("desired", nargs = 1, type = str, help = "path to the file where the desired output is saved for the follow condition")
    ...
    ax = f.add_subplot(8,1,1)
    X = np.arange(sample_size)
    ax.plot(time, inputs[args.i,:,0], linewidth = 1.0)
    ax.set_ylim(-1.0, 1.0)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.axvline(100, 0, 1, color="red", linestyle = "--", linewidth = 0.5)
    ax.set_ylabel("Value")
    ...
    ax = f.add_subplot(8,1,5)
    X = np.arange(sample_size)
    ax.plot(time[warmup:], desired_outputs[args.i,warmup:,1], linewidth = 1.0)
    ax.plot(time, outputs[args.i,:,1], color = "red", linewidth = 1.0, linestyle='dashed')
    ax.set_ylim(-0.2, 0.2)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.axvline(100, 0, 1, color="red", linestyle = "--", linewidth = 0.5)
    ...
    ax = f.add_subplot(8,1,6)
    X = np.arange(sample_size)
    ax.plot(time[:warmup], np.abs(outputs[args.i,:warmup,1]-desired_outputs[args.i,:warmup,1]), color = "green" , linewidth = 1.0, linestyle = "dashed")
    ax.plot(time[warmup:], np.abs(outputs[args.i,warmup:,1]-desired_outputs[args.i,warmup:,1]), color = "green" , linewidth = 1.0)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.axvline(100, 0, 1, color="red", linestyle = "--", linewidth = 0.5)
    ax.set_ylabel("Error")
    ...
    ax = f.add_subplot(8,1,7)
    X = np.arange(sample_size)
    ax.plot(time, outputs[args.i,:,0]*inputs[args.i,:,0], linewidth = 1.0)
    ax.plot(time, outputs[args.i,:,1], color = "red", linewidth = 1.0, linestyle='dashed')
    ax.set_ylim(-0.2, 0.2)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    ax = f.add_subplot(8,1,8)
    X = np.arange(sample_size)
    ax.plot(time, np.abs(outputs[args.i,:,1]-outputs[args.i,:,0]*inputs[args.i,:,0]), color = "green" , linewidth = 1.0)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_ylabel("Error")
    ax.axvline(100, 0, 1, color="red", linestyle = "--", linewidth = 0.5)
